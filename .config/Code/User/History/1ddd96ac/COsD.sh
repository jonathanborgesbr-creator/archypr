...
# Esta variável é usada para encontrar a sua pasta .config
DOTFILES_DIR=$(pwd)
...
# ===============================================
# 5. FINALIZAÇÕES, PERMISSÕES E DOTFILES
# ===============================================

echo "--> 5/5: Configurando permissões de GPU, layout de teclado e copiando dotfiles..."

# --- 1. CÓPIA DOS DOTFILES ---
# CRÍTICO: Copia os dotfiles para o diretório do usuário normal.
if [ -d "$DOTFILES_DIR/.config" ]; then
    # Copia o conteúdo da pasta .config do seu repositório para o .config do usuário
    su - "$USUARIO" -c "cp -r $DOTFILES_DIR/.config/* $HOME/.config/"
    echo "Dotfiles do .config copiados com sucesso para $HOME."
else
    echo "AVISO: Pasta $DOTFILES_DIR/.config não encontrada. Dotfiles NÃO copiados."
fi

# --- 2. PERMISSÕES E TECLADO ---

# Permissão para Gamescope (evita tearing/melhora desempenho)
sudo setcap 'CAP_SYS_NICE=eip' $(which gamescope)

# Adiciona o usuário ao grupo 'render' (acesso direto ao hardware gráfico)
sudo gpasswd -a $USUARIO render

# Configura o layout de teclado para X11 (usado pelo Hyprland/aplicativos)
